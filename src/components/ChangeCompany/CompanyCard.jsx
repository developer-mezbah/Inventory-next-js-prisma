      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex-1 min-w-0">
            ...
            {company.badge && (
              <span
                className={`px-2 py-1 text-xs font-medium rounded whitespace-nowrap ${
                  company.badgeColor === "orange" ? "bg-orange-100 text-orange-700" : "bg-gray-100 text-gray-700"
                }`}
              >
                {company.badge}
              </span>
          ...
          {company.lastSaleCreated && (
            <p className="text-sm text-gray-500">Last Sale Created: {company.lastSaleCreated}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 justify-end">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-medium ${company.isSyncOn ? "text-green-600" : "text-gray-600"}`}>
              {company.syncStatus}
            </span>
          </div>
