...
import { FaPlus, FaChevronDown, FaFileExcel, FaPrint, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
import { IoCalendarOutline } from 'react-icons/io5';
import { FiArrowUpRight } from 'react-icons/fi';
import { BiCheck, BiChevronDown } from 'react-icons/bi';

// Define the component structure for the stats card
const StatsCard = ({ title, amount, currency, paidAmount, percentageChange, isPositive }) => {
  // Determine color classes based on the trend (isPositive)
  const percentColor = isPositive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600';
  const arrowIcon = isPositive ? <FiArrowUpRight className="inline h-4 w-4" /> : <FiArrowUpRight className="inline h-4 w-4 transform rotate-180" />;
  ...
  return (
    <div className="max-w-xs p-6 bg-white rounded-xl shadow-lg border border-gray-100">
      ...
      <div className="flex justify-between items-start gap-4 mb-4">
        ...
        {/* Percentage Chip */}
        <div className={`flex flex-col items-center p-1 px-2 rounded-lg ${percentColor}`}>
      ...
      <div className="border-t pt-4 border-gray-100">
        <p className="text-sm text-gray-500">
          <span className="font-medium text-gray-700">Paid:</span> {paidAmount} {currency}
        </p>
      </div>
  ...
  return (
    <div ref={selectDropDownRef} className={`relative ${className}`}>
      ...
      >
        ...
        <FaChevronDown className={`ml-2 text-xs transition-transform duration-200 ${isOpen ? 'rotate-180' : 'rotate-0'}`} />
      </button>
        ...
        <div className="absolute z-10 w-full min-w-max mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
          {options.map((option) => (
            <div
              ...
              className={`p-3 text-sm cursor-pointer hover:bg-indigo-50 hover:text-indigo-700 transition-colors ${selected === option ? 'bg-indigo-100 font-medium' : ''}`}
            >
              {option}
            </div>
          ))}
  ...
  return (
    <div className="absolute top-full left-0 mt-2 z-20 p-4 bg-white border border-gray-300 rounded-xl shadow-2xl w-[250px]">
      ...
      <div className="flex items-center justify-between mb-4 p-2 bg-indigo-100 rounded-lg">
        <button onClick={goToPrevMonth} className="p-1 rounded-full text-gray-700 hover:bg-indigo-200"><FaChevronLeft size={14} /></button>
        <span className="font-semibold text-lg text-indigo-800">{monthName}</span>
        <button onClick={goToNextMonth} className="p-1 rounded-full text-gray-700 hover:bg-indigo-200"><FaChevronRight size={14} /></button>
      </div>
        ...
        {daysInMonth.map((day, index) => {
          ...
          return (
            <div
              ...
              className={`flex items-center justify-center h-8 text-sm rounded-lg cursor-pointer transition-colors duration-100 ${isToday ? 'bg-yellow-300 font-bold' : 'hover:bg-indigo-100'} ${currentDate && day.toDateString() === currentDate.toDateString() ? 'bg-blue-600 text-white shadow-md' : ''
                }`}
            >
              {day.getDate()}
            </div>
...
const PaymentOut = () => {
  ...
  const [fromDate, setFromDate] = useState(getDatesForPeriod('This Month').fromDate);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 min-h-screen bg-gray-50 font-sans">
      ...
      <div className="flex relative flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b pb-4">
        <div ref={topDropDownRef}><button onClick={() => setTopDropDown(!topDropDown)} className="text-3xl font-bold text-gray-800 mb-4 md:mb-0 flex gap-1 items-center cursor-pointer">Payment-Out <BiChevronDown /></button>
          {topDropDown && <div className={`absolute z-20 top-10 animate-in`}><DocumentMenu /></div>}</div>
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 pb-4 border-b border-gray-200">
          ...
          <div className="relative w-full sm:w-auto min-w-[120px]">
            ...
            >
              ...
              <FaChevronDown className={`ml-2 text-xs transition-transform duration-200 ${showPeriodDropdown ? 'rotate-180' : 'rotate-0'}`} />
            </button>
            {showPeriodDropdown && (
              <div ref={periodDropdownRef} className="absolute z-30 w-full min-w-max mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                {['All Purchase Invoices', 'This Month', 'Last Month', 'This Quarter', 'This Year', 'Custom'].map((option) => (
                  <div
                    key={option}
                    onClick={() => handlePeriodChange(option)}
                    className={`p-3 text-sm cursor-pointer hover:bg-indigo-50 hover:text-indigo-700 transition-colors ${period === option ? 'bg-indigo-100 font-medium' : ''}`}
                  >
                    {option}
                  </div>
                ))}
